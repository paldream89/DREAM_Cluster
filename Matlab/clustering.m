...
SetCat=1;   %category for clustering
PxSize=110; %pixel size (only used for distance threshold)
thresh=300; %distance threshold in nm for clustering search
filter=1;   %1 to filter data, 0 for no filter
Filter_Threshold=200;  %min # of localizations per cluster for filtering
%%
...
r.frame=ClusterInd;
r.cat=rcat;
outfile=sprintf('%s-cluster-40.bin',filehead);
